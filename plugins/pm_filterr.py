...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAADBQADGwUAAlUXIFU0oYAgpO6w2gI')
            return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        await query.answer("இன்னும் கொஞ்சம் வேடிக்கையாக இருக்கிறது அல்லவா?👀",show_alert=True)
